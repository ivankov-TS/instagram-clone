...
export const Aside = () => {
  return (
    <AppShell.Aside
      right="calc(50% - (min(100%, var(--container-size)) / 2) + var(--mantine-spacing-md))"
      pr={0}
      p="md"
    >
      <Stack>
        ...
        {suggestedUsers.map((item) => (
          <SuggestionItem src={item.src} nickname={item.nickname} description={item.description} />
        ))}
...
const SuggestionItem = ({ src, nickname, description }: SuggestionItemProps) => {
  return (
